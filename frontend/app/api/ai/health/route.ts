...
export async function GET(request: Request) {
  ...
  const services = await Promise.all([
    ...
    timed("Toolbox · Agent402 :3402 (591 tools, free mode)", async () => { const r = await fetch("http://127.0.0.1:3402/health", { cache: "no-store", signal: AbortSignal.timeout(4000) }); const d = await r.json(); return { ok: !!d.ok, note: d.ok ? `${d.meta?.toolCount} tools live` : "down — launchctl kickstart -k gui/$UID/com.polymarket.agent402" }; }),

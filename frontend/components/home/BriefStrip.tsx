...
// The first thing on the desk: one button, and AXIOM tells you what happened
...
export default function BriefStrip() {
  ...
  return (
    <section aria-label="Ask AXIOM" className="hud-glass rounded-2xl p-4 mb-10 flex flex-col md:flex-row md:items-center gap-3">
      ...
      <form className="flex-1 flex items-center gap-2 min-w-0" onSubmit={(e) => { e.preventDefault(); if (q.trim()) { window.dispatchEvent(new CustomEvent("axiom:jarvis-ask", { detail: q.trim() })); setQ(""); } }}>
        <label htmlFor="home-ask" className="sr-only">Ask AXIOM</label>

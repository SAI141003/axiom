...
const PAGES = [
  ...
  ["/world", "the eye, globe, world map", "God's Eye: the live globe — flights, military, satellites, quakes, launches, radio, cables, the weather book, the exchanges", "/api/eye/layers"], ["/journal", "journal", "every trade, every lesson", "/api/journal"],
...
const axiom = createSdkMcpServer({ name: "axiom", version: "2.0.0", tools: [
  ...
  tool("eye", "Drive God's Eye (/world, the live globe): focus the camera on a place (lat, lon, alt in globe radii — 0.3 city, 0.8 country, 2 whole earth), switch a layer on or off (flights, mil, sats, quakes, vessels, fires, radio, launches, cables, datacenters, dams, stations, markets), track an entity by name or callsign, release it, or set the sensor (normal, crt, nvg, flir, noir). Opens God's Eye first if the user is elsewhere. Use when Sai says 'show me', 'zoom to', 'track', 'night vision', 'what's flying over'.",
    ...
    async (cmd) => {
      ...
      return text({ opened: "/world", sent: cmd, on_screen: layers ? { stations_with_open_positions: (layers.stations ?? []).filter((s) => s.open?.length).map((s) => `${s.city}: ${s.open.length}`), exchanges_open: (layers.exchanges ?? []).filter((e) => e.open).map((e) => e.name) } : "layers unavailable",
        next: "God's Eye is on screen: flights, military ADS-B, satellites, quakes, launches, the weather book's stations and the exchanges are live layers. Say what you actually see from these facts; never invent what is on the globe." });
    }),
